...
import 'package:newtruck/api/pdf_api.dart';
...
class PdfInvoiceApi {
  ...
  static Future<File> generate(Invoice invoice) async {
    final pdf = Document();
    final imageJpg = (await rootBundle.load('assets/playstore.png')).buffer.asUint8List();
  ...
  static Widget buildInvoice(Invoice invoice) {
    ...
    final data = invoice.items.map((item) {
      final total = item.unitPrice * int.parse(item.quantity);
      return [
        item.description,
        item.quantity,
